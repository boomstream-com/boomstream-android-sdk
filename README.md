...
`mavenCentral()` уже присутствует в стандартном Android-проекте — дополнительных репозиториев и токенов не нужно.
...
```kotlin
dependencies {
    implementation("com.boomstream:player-sdk:1.0.0")
    implementation("com.boomstream:api-sdk:1.0.0")
    implementation("com.boomstream:offline-sdk:1.0.0") // опционально — только если нужны offline-загрузки
}
```
...
### Контент с защитой Clear Key DRM

Если контент защищён нативным Clear Key DRM, медиасервер проверяет специальный токен в `User-Agent` сегментных запросов. Передайте его через параметр `allowClearKeyDRMtoken`:
...
Токен приходит из `local.properties` или CI-секрета — **никогда** не хардкодьте его в исходниках. **Не логируйте `allowClearKeyDRMtoken`.**
...
```kotlin
implementation("com.boomstream:offline-sdk:1.0.0")
```
...
SDK отправляет `User-Agent: BoomstreamSDK/1.0` по умолчанию. Для идентификации вашего приложения:
...
`mavenCentral()` is already present in standard Android projects — no extra repositories or tokens needed.
...
implementation("com.boomstream:player-sdk:1.0.0")
implementation("com.boomstream:api-sdk:1.0.0")
implementation("com.boomstream:offline-sdk:1.0.0") // optional
```
...
// Clear Key DRM — token included in User-Agent for segment requests
BoomstreamPlayer(
    mediaCode = "Il4lNOfL",
    configClient = Boomstream.configClient,
    allowClearKeyDRMtoken = BuildConfig.ALLOWED_UA_KEY_TO_PLAY_AES.ifBlank { null },
