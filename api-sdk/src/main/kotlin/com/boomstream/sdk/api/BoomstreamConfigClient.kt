...
) : UserAgentTokenProvider {

    /**
     * Token from [BoomstreamOptions.userAgentToken] carried here so that `player-sdk` and
     * `offline-sdk` can use it as the default `allowClearKeyDRMtoken` when the caller passes the
     * `configClient` but omits the per-call token parameter.  `null` when no token was configured
     * at init time.
     *
     * **Not a public API.** Gated by [InternalBoomstreamApi] — only accessible from sibling SDK
     * modules that explicitly opt in.  Application code must not read this property; use
     * [BoomstreamOptions.userAgentToken] at SDK initialisation time instead.
     */
    @InternalBoomstreamApi
    override val userAgentToken: String? = userAgentToken
