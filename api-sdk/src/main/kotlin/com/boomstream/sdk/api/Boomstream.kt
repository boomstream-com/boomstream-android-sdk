...
import com.boomstream.sdk.api.BuildConfig
...
class BoomstreamSdk internal constructor(
    ...
    // Compute the effective User-Agent once at init time (immutable for the lifetime of this SDK).
    // Priority: explicit userAgent override > token-derived UA > legacy default.
    // Exposed as internal so unit tests in the same module can verify UA composition
    // without reaching into OkHttp internals via reflection.
    @Suppress("DEPRECATION")
    internal val effectiveUserAgent: String = when {
        options.userAgent != null -> options.userAgent
        options.userAgentToken != null ->
            "Boomstream Android SDK v${BuildConfig.SDK_VERSION} ${options.userAgentToken}"
        else -> "BoomstreamSDK/1.0"
    }
